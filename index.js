...
function initGlobalAPI (Vue) {
  ...
  // components with in Jud's multi-instance scenarios.
...
function createPatchFunction (backend) {
  ...
  function createElm (vnode, insertedVnodeQueue, parentElm, refElm, nested) {
    ...
    if (isDef(tag)) {
      ...
      /* istanbul ignore if */
      {
        // in Jud, the default insertion order is parent-first.
...
function updateClass (oldVnode, vnode) {
  ...
  var oldClassList = [];
  // unlike web, jud vnode staticClass is an Array
...
function getStyle (oldClassList, classList, ctx) {
  // style is a jud-only injected object
  // compiled from <style> tags in jud files
...
function updateDOMListeners (oldVnode, vnode) {
  ...
  updateListeners(on, oldOn, function (event, handler, capture) {
    if (capture) {
      console.log('Jud do not support event in bubble phase.');
      return
    }
...
function createInstance (
  ...
  // Prepare `jud` instance variable.
  var judInstanceVar = {
    config: config,
    document: document,
    require: moduleGetter
  };
  Object.freeze(judInstanceVar);
  ...
  ;['util', 'set', 'delete', 'nextTick', 'version', 'judVersion', 'config'].forEach(function (name) {
  ...
  var instanceVars = Object.assign({
    Vue: subVue,
    jud: judInstanceVar,
    __jud_require_module__: judInstanceVar.require // deprecated
  }, timerAPIs);
...
/**
 * @deprecated Just instance variable `jud.config`
 * Get instance config.
 * @return {object}
 */
